        ...
        writeClientFiles() {
            ...
            if (this.clientFramework === 'angular1') {
                ...
                generateOneOrRegenerate.call(this, () => {
                    ...
                    if (this.newPageSet) {
                        addDropdownToMenu.call(
                            this,
                            this.pageSetRouterState,
                            this.pageSetRouterState + '-' + this.pageRouterState,
                            this.pageSetGlyphIcon,
                            this.enableTranslation,
                            this.clientFramework
                        );
                    } else if (!this.regenerate) {
                        addElementToDropdown.call(
                            this,
                            this.pageSetRouterState,
                            this.pageSetRouterState + '-' + this.pageRouterState,
                            this.pageGlyphIcon,
            ...
            if (this.enableTranslation) {
                ...
                languages.forEach(language => {
                    try {
                        this.template(
                            `${CLIENT_I18N_TEMPLATES_DIR}/i18n/_page_language.json`,
                            `${constants.CLIENT_MAIN_SRC_DIR}i18n/${language}/${this.pageSetTranslationKey}.json`
                        );
                        generateOneOrRegenerate.call(this, () => {
...
function loadPageInMemory() {
    ...
    if (
        this.pageType === 'postOneToServer' ||
        this.pageType === 'loadAndSaveToServer' ||
        this.pageType === 'form' ||
    ...
    try {
            ...
            jhipsterUtils.rewriteFile(
                {
                    ...
                    splicable: [
                        `<li ng-class="{active: vm.$state.includes('${dropdownName}')}" ng-switch-when="true" uib-dropdown class="dropdown pointer">
                    ...
                    <ul class="dropdown-menu" uib-dropdown-menu>
                        <li ui-sref-active="active">
                            <a ui-sref="${routerName}" ng-click="vm.collapseNavbar()">
                                <span class="glyphicon glyphicon-${glyphiconName}"></span>&nbsp;
                                <span${enableTranslation ? ` data-translate="global.menu.${routerName}"` : ''}>${_.startCase(
                            routerName
                        )}</span>
                            </a>
                        </li>
                    ...
                    <ul class="dropdown-menu" ngbDropdownMenu>
                        <li>
                            <a class="dropdown-item" routerLink="${routerName}" routerLinkActive="active" [routerLinkActiveOptions]="{ exact: true }" (click)="collapseNavbar()">
                                <i class="fa fa-${glyphiconName}" aria-hidden="true"></i>&nbsp;
                                <span${enableTranslation ? ` jhiTranslate="global.menu.${routerName}"` : ''}>${_.startCase(
                            routerName
                        )}</span>
                            </a>
                        </li>
        ...
        this.log(
            `${chalk.yellow('\nUnable to find ') +
                ...
                dropdownName} ${chalk.yellow('not added to menu.\n')}`
        );
        this.debug('Error:', e);
        this.log('Error:', e);
    }
...
function addPageSetsModule(clientFramework) {
    ...
    try {
        ...
        this.log(
            `${chalk.yellow('\nUnable to find ') +
                appModulePath +
                chalk.yellow('. Reference to PageSets module not added to App module.\n')}`
        );
        this.log('Error:', e);
    }
}
